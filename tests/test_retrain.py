...
# build_retrain_feature.py
def test_build_retrain_features(mock_pro_path, mock_ret_path, mock_model_path) -> None:
    ...
    assert len(full_trans_train) == len(cleaned_train) + len(cleaned_valid)
    assert (mock_model_path / "retrained" / "retrain_feature_engineer_dict.pkl").exists()
    assert (mock_ret_path / "final_transformed_train.parquet").exists()
    assert (mock_ret_path / "final_transformed_test.parquet"). exists()


# retrain.py
def test_retrain_pipeline(mock_pro_path, mock_ret_path, mock_model_path) -> None:
    cleaned_train, cleaned_valid, cleaned_test = mock_cleaned_data()
    cleaned_train.to_csv(mock_pro_path / "cleaned_train.csv", index=False)
    cleaned_valid.to_csv(mock_pro_path / "cleaned_valid.csv", index=False)
    cleaned_test.to_csv(mock_pro_path / "cleaned_test.csv", index=False)
